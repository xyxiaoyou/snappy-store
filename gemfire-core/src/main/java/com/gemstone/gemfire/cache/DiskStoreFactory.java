...
public interface DiskStoreFactory
{
  ...
  /**
   * This property instructs that writes to the disk store be synchronously
   * written to disk and not to file system. (Use rwd instead of rw -
   * RandomAccessFile property)
   * 
   * @param enable
   *          true to enable synchronous writes to disk and false to disable
   * @return a reference to <code>this</code>
   */
