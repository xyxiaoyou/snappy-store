/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.catalog.ODBCProcedureColsVTI
 ...
 * this class reads from the com/pivotal/gemfirexd/internal/impl/jdbc/metadata.properties
...
public class ODBCMetadataGenerator {
	...
	private void initChanges() {
		...
		changeMap.put("getProcedures",
			Byte.valueOf(COL_RENAME_CHANGE));

		changeMap.put("getProcedureColumns",
			Byte.valueOf((byte)(COL_RENAME_CHANGE
				| TYPE_VALUE_CHANGE
				| ADD_COLUMN_CHANGE)));

		changeMap.put("getColumns",
			Byte.valueOf(TYPE_VALUE_CHANGE));
		...
		changeMap.put("getCrossReference",
			Byte.valueOf(WHERE_CLAUSE_CHANGE));
	 ...
	 */
	private void generateODBCQuery(StringBuilder queryText)
		throws IOException
	{

		// Create a string for purposes of using "indexOf"
		// calls, which aren't allowed on a StringBuilder
		// for JDBC 2.0.
		...
		// -- #1: Column renaming.
		StringBuilder outerQueryText = new StringBuilder();
		...
		ArrayList colDefs = new ArrayList();
	      // GemStone changes BEGIN
	        ...
	        pos = -1;
                do {
                  pos = getSelectColDefinitions(queryText, (pos == -1 ? colDefs : null),
                      pos);
	 ...
	 *	PLACE in the received StringBuilder.  True is returned if
	 *	at least one column was renamed; false otherwise.
	 */
	private boolean renameColsForODBC(String queryName, StringBuilder queryText) {
	 ...
	 */
	private void renameColForODBC(StringBuilder queryText,
	 ...
	 *  elements, "A" and "B".
	 * @param newQueryText StringBuilder to which the generated
	...
	private void generateSELECTClause(String queryName,
		ArrayList selectColDefs, StringBuilder newQueryText)
	{
	 ...
	 * @param beginOffset default is -1 indicating there is no
	 *      other select. If jdbc query contains UNION, beginOffset
	 *      should be passed with the offset of the UNION token.
	 ...
	 */
	private int getSelectColDefinitions(StringBuilder queryText,
		ArrayList colDefList, int beginOffset)
	{
		...
		// Move beyond the "SELECT" keyword, if there is one.
                // GemStone changes BEGIN
                int start = beginOffset == -1 ? query.indexOf("SELECT") : query.indexOf("SELECT", beginOffset);
                /*(original code) int start = query.indexOf("SELECT");*/
                // GemStone changes END
    ...
    private void substitutePatternWhere(String oldPattern, String newPattern,
        ...
        //Rather, verify that
        //classes/com/pivotal/gemfirexd/internal/impl/jdbc/metadata.properties is
