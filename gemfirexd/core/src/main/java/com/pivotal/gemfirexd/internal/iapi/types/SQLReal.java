/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.types.SQLReal
...
// GemStone changes BEGIN
  ...
  public int getLengthInBytes(DataTypeDescriptor dtd) {
    if (!isNull()) {
      // getLength() returns 16 (!!)
      // We only need 4 bytes.
      return Float.SIZE >>> 3;
    }
    return 0;
  }
  ...
  public int computeHashCode(int maxWidth, int hash) {
    assert !isNull();
    final int bits = Float.floatToIntBits(this.value);
    return ResolverUtils.addIntToBucketHash(bits, hash, getTypeFormatId());
  }

  static final float getAsFloat(final byte[] inBytes, final int offset) {
