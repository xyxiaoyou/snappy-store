/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.InsertNode
...
public final class InsertNode extends DMLModStatementNode
{
			...
			Object isPutDML)
	{
		...
		this.isPutDML = ((Boolean) isPutDML).booleanValue();
	}
		...
		if (SanityManager.DEBUG)
		{
            try {
                return ( (targetTableName!=null) ? targetTableName : targetVTI.getTableName() ).toString() + "\n"
                    + targetProperties + "\n"
                    + super.toString();
            } catch (com.pivotal.gemfirexd.internal.iapi.error.StandardException e) {
...
// GemStone changes BEGIN
			...
			mb.callMethod(VMOpcode.INVOKEINTERFACE, (String) null, "getGemFireInsertResultSet", ClassName.ResultSet, 2);
// GemStone changes END
		}
		else
		{
...
// GemStone changes BEGIN
  ...
  public QueryInfo computeSubSelectQueryInfo(QueryInfoContext qic)
    ...
    if (!this.resultSetInfoInitialized && this.resultSet != null) {
      ...
      if (qInfo.isSelect()) {
        SanityManager.ASSERT(qInfo instanceof SelectQueryInfo);
        resultSetInfo = (SelectQueryInfo)qInfo;
      }
    }
  ...
  public String getNonVTITargetTableName() {
    ...
    return targetTableName.getFullTableName();
  }
  ...
  public Set<Object> getSingleHopInformation(
      GenericParameterValueSet parameterValueSet,
      GenericPreparedStatement preparedStmt, GfxdPartitionResolver resolver, TableDescriptor ttd)
      throws StandardException {
    ResultColumnList rcl = this.resultColumnList;
    return getSHOPInformation(rcl, resolver, ttd);
  }
  ...
  public Set<Object> getSHOPInformation(ResultColumnList topLevelRCL,
    ...
    for (int i = 0; i < numColumns; i++) {
      //printColumnInformation((ResultColumn)rcl.elementAt(i), i);
      //printColumnInformation2((ResultColumn)topLevelRCL.elementAt(i), i);
      // System.out.println("type=" + types[i]);
      //System.out.println("*************************************");
      ResultColumn rc = (ResultColumn)rcl.elementAt(i);
      String colName = rc.getActualName();
      boolean partOfPartitioningColumn = resolver.isUsedInPartitioning(colName);
      ColumnRoutingObjectInfo croi = getRoutingObjectInfo(rc, resolver, partOfPartitioningColumn, colName, ttd);
      if (croi != null) {
        set.add(croi);
      }
      else if (partOfPartitioningColumn) {
        return null;
      }
    }
    if (set.size() > 1) {
  ...
  private ColumnRoutingObjectInfo getRoutingObjectInfo(ResultColumn rc,
      GfxdPartitionResolver resolver, boolean partOfPartitioningColumn, String colName, TableDescriptor ttd) throws StandardException {
    if (partOfPartitioningColumn) {
      return getAppropriateRoutingObjectInfo(rc, resolver, colName, ttd);
    }
    return null;
  }

  private ColumnRoutingObjectInfo getAppropriateRoutingObjectInfo(
      ResultColumn rc, GfxdPartitionResolver resolver, String colName, TableDescriptor ttd) throws StandardException {
    ...
    int paramNum = isParameterNode(vn);
    Object constantValue = null;
    if (vn instanceof ConstantNode) {
      constantValue = ((ConstantNode)vn).getValue().getObject();
    }
    boolean isParameter = paramNum > 0 ? true : false;
    if (!isParameter) {
      constantValue = rc.getConstantValueAsObject();
      if (constantValue == null) {
        if (vn != null) {
          ResultColumn internalRC = vn.getSourceResultColumn();
          if (internalRC != null) {
            constantValue = internalRC.getConstantValueAsObject();
            if (constantValue == null) {
              ...
              else if (vn instanceof ParameterNode) {
                isParameter = true;
                paramNum = ((ParameterNode)vn).getParameterNumber();
              }
              else {
                // TODO: Cast node can be handled. But have to look at all the possibilities
                return null;
              }
            }
          }
    ...
    return ComparisonQueryInfo.getAppropriateRoutingObjectInfo(vn, type,
        isParameter, paramNum, constantValue, resolver, maxWidth);
  }
  ...
  private void printColumnInformation(ResultColumn rc, int idx) {
    ...
    printColumnInformation2(rc, idx);
  }
  ...
  private void printColumnInformation2(ResultColumn rc, int idx) {
    ...
    //System.out.println("col expression: " + rc.getExpression());
    //System.out.println("col expression source: " + ((ColumnReference)rc.getExpression()).getSource());
  }
// GemStone changes END
