/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.catalog.DataDictionaryImpl
...
import com.pivotal.gemfirexd.internal.iapi.services.sanity.AssertFailure;
...
import com.pivotal.gemfirexd.internal.impl.jdbc.LOBStoredProcedure;
...
public abstract class DataDictionaryImpl
	...
	implements DataDictionary, CacheableFactory, ModuleControl, ModuleSupportable,java.security.PrivilegedAction
{
	...
	*/
// GemStone changes BEGIN
	// increased visibility to protected
	protected /* private */ static final String[][] SYSFUN_FUNCTIONS = {
// GemStone changes END
			...
			{"PI", "DOUBLE", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "PI()", null},
			...
			{"LOG10", "DOUBLE", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "LOG10(double)", "DOUBLE"},
			...
			{"SIGN", "INTEGER", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "SIGN(double)", "DOUBLE"},
			{"RANDOM", "DOUBLE", "java.lang.StrictMath", "random()", null},
			{"RAND", "DOUBLE", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "RAND(int)", "INTEGER"}, // Escape function spec.
			{"COT", "DOUBLE", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "COT(double)", "DOUBLE"},
			{"COSH", "DOUBLE", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "COSH(double)", "DOUBLE"},
			{"SINH", "DOUBLE", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "SINH(double)", "DOUBLE"},
			{"TANH", "DOUBLE", "com.pivotal.gemfirexd.internal.catalog.SystemProcedures", "TANH(double)", "DOUBLE"}
	};
...
//                                    GemStone changes END                                                   
     ...
     **/
// GemStone changes BEGIN
    // increased the visibility to protected
    protected static final String[] systemSchemaNames = {
    /* (original code) private static final String[] systemSchemaNames = { */
// GemStone changes END
	...
	*/
	...
	private static final String[] sysUtilProceduresWithPublicAccess = { 
	                                                                                        //GemStone changes BEGIN
												//"SET_RUNTIMESTATISTICS",
                                                                                                "SET_RUNTIMESTATISTICS",
	                                                                                        "SET_STATEMENT_STATISTICS",
                                                                                                "SET_STATISTICS_SUMMARY",
                                                                                                //GemStone changes END
	...
	*/
			...
			throws StandardException
	{
		 ...
		 */
		CacheFactory cf =
				(CacheFactory) Monitor.startSystemModule(com.pivotal.gemfirexd.internal.iapi.reference.Module.CacheFactory);
		OIDTdCache =
		...
		try
		{
		  ...
		  // Gemstone changes Begin.
		  // Rahul : changed the call to get a boot time transaction controller.
			...
			if (create) {
				...
				userDefinedCollation = startParams.getProperty(
						com.pivotal.gemfirexd.Attribute.COLLATION, Property.UCS_BASIC_COLLATION);		
			...
			if (create) {
				String userName = IdUtil.getUserNameFromURLProps(startParams);
                                // GemStone changes BEGIN
                                String bi = (String)startParams
                                    .getProperty(GfxdConstants.PROPERTY_BOOT_INDICATOR);
                                
                                boolean isOwner = true;
                                if (bi == null) {
                                  assert GemFireStore.getBootedInstance() != null:
                                    "Boot inidicator is expected if database is not booted";
                                  bi = GfxdConstants.BT_INDIC.FIRSTCONNECTION.toString();
                                  isOwner = false;
                                }
                                
                                if (isOwner == false) {
                                  String rstr = null;
                                  while ((rstr = GemFireXDUtils.getRandomString(
                                      false)).equals(userName));
                                  userName = "garbled_owner_" + rstr;
                                }
                                // GemStone changes END
				authorizationDatabaseOwner = IdUtil.getUserAuthorizationId(userName);
                                ...
                                if (GemFireXDUtils.TraceFabricServiceBoot) {
                                  SanityManager.DEBUG_PRINT(GfxdConstants.TRACE_FABRIC_SERVICE_BOOT,
                                      "Distributed Member owner = " + authorizationDatabaseOwner
                                          + ", booted with " + bi);
                                }
				...
				// GemStone change: create this when LCC is available
				//createSystemSps(bootingTC);
                // create the SYSCS_UTIL system procedures)
                create_procedures(bootingTC,newlyCreatedRoutines);
// GemStone changes BEGIN
                this.createGfxdSchemaAndInbuiltProcedures(bootingTC, newlyCreatedRoutines);
// GemStone changes END
				...
				if (PropertyUtil.getSystemBoolean(com.pivotal.gemfirexd.Property.SQL_AUTHORIZATION))
				{
					bootingTC.setProperty(com.pivotal.gemfirexd.Property.SQL_AUTHORIZATION,"true",true);
				...
				if (Boolean.valueOf
					...
					// (e.g. gemfirexd.sql-authorization) which
    ...
    private CacheManager getPermissionsCache() throws StandardException
    {
        ...
        {
          // GemStone changes BEGIN
          // ensuring only one permission cache gets booted up instead of multiple instances. #43245
          synchronized (this) {
          // GemStone changes END
	...
	*/
	...
	/**
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary#startReading
	 *
	...
	/* @see com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary#doneReading */
	...
	/*
	 * @see com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary#startWriting
	 *
	 * @exception StandardException		Thrown on error
	 */
	...
	/* @see com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary#transactionFinished */
...
// GemStone changes END
		...
		throws StandardException
	{
		...
		//check whether any jar is installed in this schema
		GfxdJarResource jarResource = Misc.getMemStoreBooting()
		          .getJarFileHandler();
		if (jarResource != null) {
		  for (String alias : jarResource.getNameToIDMap().keySet()) {
		    int dotIndex = alias.indexOf('.');
                    if (dotIndex != -1) {
                      String schema = alias.substring(0, dotIndex);
                      if (sd.getSchemaName().equals(schema)) {
                        return false;
                      }
                    }
		  }
...
// GemStone changes BEGIN
					ScanController.GT, null);     			// stopSearchOperation
// GemStone changes END
	...
	*/
		...
		throws StandardException
	{
		...
		ScanController sc = tc.openScan(
			...
			// GemStone changes BEGIN
			0, null);                           // stopSearchOperation - none
		        // GemStone changes END
...
//		// TMP:
//		if (stmtName.charAt(0) == '"') {
//		  stmtName = stmtName.substring(1, stmtName.length() - 1);
//		}
		/*
		** If we didn't get a schema descriptor, we had better
		** have a system table.
	...
	{
// GemStone changes BEGIN
	  // no longer invalidate SPS under any circumstances
	  // else can lead to deadlocks during LOB or other
	  // SPS executions due to inversion of locks where
	  // it will try DD write lock during DML execution
	  // (see #45609, #47336, #47824)
	  /* (original code)
...
// GemStone changes BEGIN
			scanController.fetch(indexRow1);
			baseRowLocation = (RowLocation)	indexRow1.getColumn(
			    indexRow1.nColumns());
			baseRowLocation = heapCC.fetch(
			    baseRowLocation, outRow, (FormatableBitSet)null, true);
			boolean base_row_exists = baseRowLocation != null;
			if (base_row_exists) {
			  indexRow1.setColumn(indexRow1.nColumns(),
			      baseRowLocation);
			}
// GemStone changes END
            if (SanityManager.DEBUG)
			...
			throws StandardException
	{
		...
		*/
		...
		try
		{
			while (scanController.fetchNext(outRow))
			{
				SubConstraintDescriptor subCD = null;
			...
			throws StandardException
	{
		...
		try
		{
			...
			while (scanController.fetchNext(indexRow1))
			{	
				baseRowLocation = (RowLocation)	
                    indexRow1.getLastColumn();
// GemStone changes BEGIN
				// get the row and grab the uuid
				baseRowLocation = heapCC.fetch(
				    baseRowLocation, rowTemplate, columnToGetSet, true);
				boolean base_row_exists = baseRowLocation != null;
				if (base_row_exists) {
				  indexRow1.setColumn(indexRow1.nColumns(),
				      baseRowLocation);
...
// GemStone changes BEGIN
			...
			final Long hashKey = Long.valueOf(cd.getConglomerateNumber());
			/* (original code)
	...
	*/
		...
		throws StandardException
	{
		...
		ConglomerateDescriptor	conglomerateDescriptor = bootstrapOneIndex
			( sd, tc, ddg, ti, indexNumber, heapConglomerateNumber, null ); // check this rahul
				...
				throws StandardException
	{
         ...
         */
		if (scanController.fetchNext(indexTemplateRow))
		{
			RowLocation	baseRowLocation;


			baseRowLocation = (RowLocation)	indexTemplateRow.getLastColumn();
...
// GemStone change BEGIN
			baseRowLocation = heapCC.fetch(
			    baseRowLocation, row, (FormatableBitSet)null, true);
			boolean base_row_exists = baseRowLocation != null;
			if (base_row_exists) {
			  indexTemplateRow.setColumn(indexTemplateRow.nColumns(),
			      baseRowLocation);
			}
// GemStone change END
            if (SanityManager.DEBUG)
	...
	*/
						...
						throws StandardException
	{
		 ...
		 */
// GemStone changes BEGIN
    TableDescriptor td = ti.getTableDescriptor();
    if (td == null) {
...
// GemStone changes BEGIN
	 ...
	 */
	public NumberDataValue getSetAutoincrementValue(RowLocation[] rlarray, int index,
											TransactionController tc,
											boolean doUpdate,
			...
			*/          
            heapCC = 
                tc.openConglomerate(
                    ...
                    TransactionController.ISOLATION_REPEATABLE_READ);
// GemStone changes BEGIN
            rlarray[index] = heapCC.fetch(rlarray[index], row.getRowArray(),
                columnToRead, wait, false);
// GemStone changes END
...
// GemStone changes BEGIN
                heapCC.replace(rlarray[index], row.getRowArray(), columnToUpdate);
// GemStone changes END
            }
...
// GemStone changes BEGIN
		indexProperties.put("ddIndex", "true");
		indexProperties.put(GfxdConstants.PROPERTY_SCHEMA_NAME,sd.getSchemaName());
// GemStone changes END

		/* Create and add the conglomerate (index) */
		conglomId = tc.createConglomerate(
//GemStone changes BEGIN
			GfxdConstants.LOCAL_SORTEDMAP_INDEX_TYPE,/*"BTREE", // we're requesting an index conglomerate*/
//GemStone changes END
...
// GemStone changes END
    ...
    properties.setProperty(GfxdConstants.PROPERTY_TABLE_NAME, name);
	...
	private final TupleDescriptor getDescriptorViaIndexMinion(
		...
		ExecIndexRow keyRow,
		ScanQualifier [][] scanQualifiers,
		ScanQualifier[][] outQualifiers, // GemStone addition
		TabInfoImpl ti,
		TupleDescriptor parentTupleDescriptor,
		List list,
		boolean forUpdate,
...
// GemStone changes BEGIN
			scanController.fetch(indexRow1);
			baseRowLocation = (RowLocation)	indexRow1.getLastColumn();
            ...
            try
            {
                baseRowLocation = heapCC.fetch(
                    baseRowLocation, outRow, (FormatableBitSet)null, true);
                base_row_exists = baseRowLocation != null;
                // check for any additional qualifiers on output row
                if (outQualifiers != null &&
                    !RowFormatter.qualifyRow(outRow, false, outQualifiers)) {
                  continue;
                }
                if (base_row_exists) {
                  indexRow1.setColumn(indexRow1.nColumns(),
...
// GemStone changes BEGIN
                    // never do exit() in GemFire since it will start invoking
                    // shutdown hooks and mask the real source; using exit()
...
// GemStone changes BEGIN
  public String getBuiltinVTIClass(String schemaName, String tableName)
    ...
    /* First check to see if this is a system VTI. Note that if no schema was
     * specified then the call to "td.getSchemaName()" will return the current
     * schema.
     */
    if (SchemaDescriptor.STD_SYSTEM_DIAG_SCHEMA_NAME.equals(schemaName)) {
      String[][] vtiMappings = DIAG_VTI_TABLE_CLASSES;

      for (int i = 0; i < vtiMappings.length; i++) {
        String[] entry = vtiMappings[i];
        if (entry[0].equals(tableName)) {
          return entry[1];
        }
      }
    }
    ...
    protected void debugGenerateInfo(StringBuilder strbuf,
    /* private void debugGenerateInfo(StringBuilder strbuf, */
// GemStone changes END
...
//EXCLUDE-END-lockdiag-
	 ...
	 * @exception StandardException		Thrown on error
	 */
	public /* GemStone change: originally: protected */ TupleDescriptor getDescriptorViaHeap(
						ScanQualifier [][] scanQualifiers,
						TabInfoImpl ti,
						TupleDescriptor parentTupleDescriptor,
						List list)
	 ...
	 * @exception StandardException		Thrown on error
	 */
	public TabInfoImpl getNonCoreTI(int catalogNumber)
		throws StandardException
	{
		TabInfoImpl	ti = getNonCoreTIByNumber(catalogNumber);
		...
		if (retval == null)
		{
			...
			switch (catalogNumber)
			{
			  ...
			  case SYSROLES_CATALOG_NUM:
				...
				break;
//				    GemStone changes Begin                                                   
			  case SYSDISKSTORES_CATALOG_NUM:
                            retval = new TabInfoImpl(new GfxdSYSDISKSTORESRowFactory( luuidFactory,
                                exFactory, dvf));
				break;
		          case GATEWAYSENDERS_CATALOG_NUM:
		           retval = new TabInfoImpl(new GfxdSysGatewaySenderRowFactory( luuidFactory,
		              exFactory, dvf));
		              break;
		          case ASYNCEVENTLISTENERS_CATALOG_NUM:
	                   retval = new TabInfoImpl(new GfxdSysAsyncEventListenerRowFactory( luuidFactory,
	                      exFactory, dvf));
	                      break;
		          case GATEWAYRECEIVERS_CATALOG_NUM:
	                   retval = new TabInfoImpl(new GfxdSysGatewayReceiverRowFactory( luuidFactory,
	                      exFactory, dvf));
	                      break;
	                  case SYSHDFSSTORES_CATALOG_NUM:
	                            retval = new TabInfoImpl(new GfxdSYSHDFSSTORESRowFactory( luuidFactory,
	                                exFactory, dvf));
	                                break;
//			      GemStone changes END
			}
...
// GemStone changes END
	 ...
	 * @exception  StandardException  Standard exception policy.
     **/
    protected final UUID createSystemProcedureOrFunction(
    String                  routine_name,
    UUID                    schema_uuid,
    String[]                arg_names,
    ...
    TransactionController   tc, String procClass,
    boolean isRestricted /* GemStoneAddition */)
        throws StandardException
    {
        int num_args = 0;
        if (arg_names != null)
...
// GemStone changes BEGIN
        if (isRestricted) {
          addRestrictedRoutine(routine_uuid);
        }
// GemStone changes END
		return routine_uuid;
    }
    ...
    short                   routine_sql_control,
    TypeDescriptor          return_type,
    HashSet newlyCreatedRoutines, TransactionController   tc)
        throws StandardException
    {
        ...
        return_type, newlyCreatedRoutines, tc, "com.pivotal.gemfirexd.internal.catalog.SystemProcedures",
	 ...
	 * @exception  StandardException  Standard exception policy.
     **/
    private final void create_procedures(
        ...
        throws StandardException
    {
		...
		*/
        ...
        // void SYSCS_UTIL.SET_DATABASE_PROPERTY(
        ...
        // void SYSCS_UTIL.COMPRESS_TABLE(varchar(128), varchar(128), SMALLINT)
        {
            // procedure argument names
        ...
        // void SYSCS_UTIL.CHECKPOINT_DATABASE()
        {
            createSystemProcedureOrFunction(
                "CHECKPOINT_DATABASE",
                sysUtilUUID,
        ...
        // void SYSCS_UTIL.FREEZE_DATABASE()
        {
            createSystemProcedureOrFunction(
                "FREEZE_DATABASE",
                sysUtilUUID,
        ...
        // void SYSCS_UTIL.UNFREEZE_DATABASE()
        {
            createSystemProcedureOrFunction(
                "UNFREEZE_DATABASE",
                sysUtilUUID,
        ...
        // void SYSCS_UTIL.BACKUP_DATABASE(varchar Limits.DB2_VARCHAR_MAXWIDTH)
        {
            // procedure argument names
        ...
        // void SYSCS_UTIL.BACKUP_DATABASE_AND_ENABLE_LOG_ARCHIVE_MODE(
        ...
        // void SYSCS_UTIL.DISABLE_LOG_ARCHIVE_MODE(smallint)
        {
            // procedure argument names
        ...
        // void SYSCS_UTIL.SET_RUNTIMESTTISTICS(smallint)
        {
            // procedure argument names
        ...
        // void SYSCS_UTIL.SET_STATISTICS_TIMING(smallint)
        {
            // procedure argument names
            String[] arg_names = {"ENABLE"};
        ...
        // GemStone changes BEGIN
        // void SYSCS_UTIL.SET_STATEMENT_STATISTICS(smallint)
        {
            // procedure argument names
            String[] arg_names = {"ENABLE"};
            ...
            routine_uuid = createSystemProcedureOrFunction(
                            ...
                            tc
// GemStone changes BEGIN
                            ,GfxdSystemProcedures.class.getName(), false
// GemStone changes END
                            );
        ...
        // SYSCS_UTIL.GET_DATABASE_PROPERTY(varchar(128))
        ...
        // SMALLINT SYSCS_UTIL.CHECK_TABLE(varchar(128), varchar(128))
        {
            // procedure argument names
        ...
        // CLOB SYSCS_UTIL.GET_RUNTIMESTATISTICS()
        {
		...
		*/
		...
		/*  IMPORT_DATA(IN SCHEMANAME VARCHAR(128), 
...
// GemStone changes END
    ...
    { 
        ...
        // void SYSCS_UTIL.INPLACE_COMPRESS_TABLE(
        //     IN SCHEMANAME        VARCHAR(128), 
        //     IN TABLENAME         VARCHAR(128),
    ...
    private void create_10_3_system_procedures_SYSIBM(
        ...
        UUID schema_uuid = getSysIBMSchemaDescriptor().getUUID();
        {
            ...
            routine_uuid = createSystemProcedureOrFunction(
                ...
                TypeDescriptor.INTEGER,
                newlyCreatedRoutines,
                tc, "com.pivotal.gemfirexd.internal.impl.jdbc.LOBStoredProcedure",
                false /* GemStoneAddition */);
        }
        {
            UUID routine_uuid = null;
    ...
    throws StandardException
    {
        ...
        UUID  sysUtilUUID = getSystemUtilSchemaDescriptor().getUUID();
        // void SYSCS_UTIL.SET_EXPLAIN_CONNECTION(smallint mode)
        {
            // procedure argument names
            String[] arg_names = {"ENABLE"};
...
// GemStone changes END
        ...
        // SMALLINT SYSCS_UTIL.GET_XPLAIN_CONNECTION()
        {

            routine_uuid = createSystemProcedureOrFunction(
                            "GET_EXPLAIN_CONNECTION",
                            sysUtilUUID,
                            null,
                            null,
                            0,
...
// GemStone changes END
        ...
        throws StandardException
    {
         ...
         */
// GemStone changes BEGIN
        // now an extended version registered from GfxdDataDictionary
        /* (original code)
        {
            // procedure argument names
            String[] arg_names = {"schemaName", "tableName", "fileName",
...
// GemStone changes END
        ...
        // void SYSCS_UTIL.RELOAD_SECURITY_POLICY()
        {
            createSystemProcedureOrFunction(
        ...
        // void SYSCS_UTIL.SET_USER_ACCESS(USER_NAME VARCHAR(128),
        ...
        // VARCHAR(128) SYSCS_UTIL.GET_USER_ACCESS(USER_NAME VARCHAR(128))
        {               
            TypeDescriptor[] arg_types = { CATALOG_TYPE_SYSTEM_IDENTIFIER };
        ...
        // void SYSCS_UTIL.EMPTY_STATEMENT_CACHE()
        {               
            createSystemProcedureOrFunction(
	...
	*/
	...
	private final synchronized Properties getQueryDescriptions(boolean net) {
		spsSet = net ? "metadata_net.properties" : "/com/pivotal/gemfirexd/internal/impl/jdbc/metadata.properties";
	...
	/* @see com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary#getTablePermissions */
	...
	/* @see com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary#getColumnPermissions */
	...
	/* @see com.pivotal.gemfirexd.internal.iapi.sql.dictionary.DataDictionary#getRoutinePermissions */
...
// GemStone changes BEGIN
                final int ldapColumnNum = rf.getLdapGroupColumnNumber();
                if (ldapColumnNum > 0) {
                  // if adding permission without LDAP group on existing with
                  // LDAP group then remove LDAP group (i.e. retain *stronger*)
                  if (perm.getLdapGroup() == null
                      && existingRow.getColumn(ldapColumnNum) != null) {
                    existingRow.setColumn(ldapColumnNum, new SQLChar(null));
                    colsChanged[ldapColumnNum - 1] = true;
                    changedColCount++;
                  }
                }
// GemStone changes END
	...
	{
			{"LOCK_TABLE", "com.pivotal.gemfirexd.internal.diag.LockTable"},
			{"STATEMENT_CACHE", "com.pivotal.gemfirexd.internal.diag.StatementCache"},
			{"TRANSACTION_TABLE", "com.pivotal.gemfirexd.internal.diag.TransactionTable"},
			{"ERROR_MESSAGES", "com.pivotal.gemfirexd.internal.diag.ErrorMessages"},
	...
	{
			{"SPACE_TABLE", "com.pivotal.gemfirexd.internal.diag.SpaceTable"},
			{"ERROR_LOG_READER", "com.pivotal.gemfirexd.internal.diag.ErrorLogReader"},
			{"STATEMENT_DURATION", "com.pivotal.gemfirexd.internal.diag.StatementDuration"},
	};
...
// GemStone changes BEGIN
  ...
  /**
   * List of routines in system schemas that are restricted and require explicit
   * GRANT to be exeucted by normal users.
   */
  private final HashSet<UUID> restrictedSysRoutines = new HashSet<UUID>();

  /**
   * List of special routines that will do their own authorization for which
   * the regular authorization should be skipped (e.g. CHANGE_PASSWORD
   * which should allow a user to change own password)
   */
  private final HashSet<UUID> bypassRoutines = new HashSet<UUID>();

  protected final void addRestrictedRoutine(UUID routineID) {
    this.restrictedSysRoutines.add(routineID);
  }

  protected final void addBypassRoutine(UUID routineID) {
    this.bypassRoutines.add(routineID);
  }
   ...
   */
  @Override
  public boolean isRestrictedRoutine(String schemaName, UUID routineID) {
    return schemaName.equals(SchemaDescriptor.STD_SQLJ_SCHEMA_NAME)
        || schemaName.equals(SchemaDescriptor.STD_SYSTEM_UTIL_SCHEMA_NAME)
        || (routineID != null && this.restrictedSysRoutines.contains(routineID));
  }

  public boolean bypassRoutineAuthorization(UUID routineID) {
    return this.bypassRoutines.contains(routineID);
  }

  public Dependable getAsyncEventListenerDescriptor(
      UUID uuid) throws StandardException {
    TabInfoImpl ti = getNonCoreTI(SYSDEPENDS_CATALOG_NUM);
    String id = (String)Misc.getMemStore().getUUID_IDMap().get(uuid);

    return new GfxdAsyncEventListenerDescriptor(id, uuid);
  }
   ...
   */
  @Override
  public List<PermissionsDescriptor> getLDAPDescriptorsHavingPermissions(
      PermissionsDescriptor perm, int catalogNum, int indexNum,
      int groupColumnNum, TransactionController tc) throws StandardException {
    ArrayList<PermissionsDescriptor> descriptors = new ArrayList<>();
    TabInfoImpl ti = getNonCoreTI(catalogNum);
    PermissionsCatalogRowFactory rowFactory = (PermissionsCatalogRowFactory)ti
        .getCatalogRowFactory();
    ExecIndexRow keyRow = rowFactory.buildIndexKeyRow(indexNum, perm);

    // additional qualifier for ldap group name (on output row, not index scan
    // since this is not an index column)
    ScanQualifier[][] qualifier = null;
    String ldapGroup = perm.getLdapGroup();
    if (ldapGroup != null) {
      qualifier = exFactory.getScanQualifier(1);
      qualifier[0][0].setQualifier(groupColumnNum - 1, null, // No column name
          new SQLVarchar(ldapGroup), Orderable.ORDER_OP_EQUALS, false, false,
          false);
    }
    getDescriptorViaIndexMinion(indexNum, keyRow, null, qualifier, ti, null,
        descriptors, false, getTransactionCompile());
    return descriptors;
  }
   ...
   */
  @Override
  public void getAllLDAPDescriptorsHavingPermissions(String ldapGroup,
      int catalogNum, int groupColumnNum, TransactionController tc,
      List<? extends PermissionsDescriptor> outDescriptors)
          throws StandardException {
    TabInfoImpl ti = getNonCoreTI(catalogNum);
    PermissionsCatalogRowFactory rowFactory = (PermissionsCatalogRowFactory)ti
        .getCatalogRowFactory();

    // additional qualifier for ldap group name
    ScanQualifier[][] qualifier = null;
    if (ldapGroup != null) {
      qualifier = exFactory.getScanQualifier(1);
      qualifier[0][0].setQualifier(groupColumnNum - 1, null, // No column name
    ...
    getDescriptorViaHeap(qualifier, ti, null, outDescriptors);
  }
// GemStone changes END
...
// GemStone changes BEGIN
	abstract void createGfxdSchemaAndInbuiltProcedures(
	    TransactionController tc, HashSet newlyCreatedRoutines)
	    throws StandardException;

	public final ConglomerateDescriptor[] getAllConglomerateDescriptors()
	    throws StandardException {
	  ScanController scanController;
	  TransactionController tc;
	  ExecRow outRow;
	  TabInfoImpl ti = coreInfo[SYSCONGLOMERATES_CORE_NUM];
	  SYSCONGLOMERATESRowFactory rf = (SYSCONGLOMERATESRowFactory)ti
	      .getCatalogRowFactory();

	  ConglomerateDescriptorList cdl = new ConglomerateDescriptorList();
	  getDescriptorViaHeap(null, ti, null, cdl);
