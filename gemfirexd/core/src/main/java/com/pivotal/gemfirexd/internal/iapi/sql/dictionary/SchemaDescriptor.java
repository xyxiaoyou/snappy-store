/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.sql.dictionary.SchemaDescriptor
	...
	implements UniqueTupleDescriptor, Provider
{
	...
	*/
     ...
     *
     * See com.pivotal.gemfirexd.internal.impl.sql.conn.GenericLanguageConnectionContext#getSystemSchemaName
     */
     ...
     *
     * See com.pivotal.gemfirexd.internal.impl.sql.conn.GenericLanguageConnectionContext#getDefaultSchemaName
     */
...
// GemStone changes BEGIN
    // using value from GfxdConstants to keep this uniform and GfxdConstants
    // cannot use this since it is used by tools etc. and must be kept pristine
    public static final String STD_DECLARED_GLOBAL_TEMPORARY_TABLES_SCHEMA_NAME
        = com.pivotal.gemfirexd.internal.engine.GfxdConstants.SESSION_SCHEMA_NAME;
    /* (original code)
...
// GemStone changes END
	...
	*/
	 ...
	 */
// GemStone changes BEGIN
	// some GemFireXD routines in SYS schema now should not be public
	public boolean isSchemaWithGrantableRoutines(UUID routineID) {
	  if (!isSystem) {
	    return true;
	  }
	  return getDataDictionary().isRestrictedRoutine(name, routineID);
	/* (original code)
...
// GemStone changes BEGIN
  ...
  private java.util.SortedSet<String> defaultServerGroups =
    new java.util.TreeSet<String>();

  /**
   * Get the default server groups for this schema.
   * 
   * @return the set of default server groups
   */
  public java.util.SortedSet<String> getDefaultServerGroups() {
    return this.defaultServerGroups;
  }

  /**
   * Set the default server groups for this schema.
   * 
   * @param serverGroups
   *          the default server groups attribute
   */
  public void setDefaultServerGroups(java.util.SortedSet<String> serverGroups) {
    if (serverGroups != null) {
      this.defaultServerGroups = serverGroups;
    }
  }
