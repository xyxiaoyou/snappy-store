/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.iapi.services.context.ContextService
...
// GemStone changes BEGIN
	/* (original code) private HashSet allContexts; */
	private ConcurrentHashSet<ContextManager> allContexts;

	// flags returned by addToThreadList
	private static final int ALREADY_CURRENT = 0x1;
	private static final int CURRENT_NULL = 0x2;
	private static final int OLD_INACTIVE_CM_OVERWRITTEN = 0x4;
	private static final int IS_ADDED = 0x8;
	private static final int SERVICE_STOPPED = 0x10;
	private static final int ALREADY_CURRENT_IS_ADDED = 0x11;
...
// GemStone changes BEGIN
                allContexts = new ConcurrentHashSet<ContextManager>();
                /* (original code)
...
// GemStone changes BEGIN
		// changed to Dequeue from Stack
		final java.util.ArrayDeque<?> stack = (java.util.ArrayDeque<?>)list;
		/* (original code) java.util.Stack stack = (java.util.Stack) list; */
// GemStone changes END
...
// GemStone changes BEGIN
		final java.util.ArrayDeque<?> stack = (java.util.ArrayDeque<?>)tcl.get();
		/* (original code) java.util.Stack stack = (java.util.Stack) tcl.get(); */
// GemStone changes END
...
// GemStone changes BEGIN
		if (GemFireXDUtils.TraceCM) {
		  SanityManager.DEBUG_PRINT(GfxdConstants
		      .TRACE_CONTEXT_MANAGER, "Reset active thread to "
		      + cm.activeThread + " activeCount=" + cm.activeCount
		      + " for cm " + ArrayUtils.objectRefString(cm)
		      +  " with conn " + EmbedConnectionContext
		          .getEmbedConnection(cm));
		}
		// switch transaction state in case there was a switch
		// between ContextManagers on the current thread
		if (nextCM != cm) {
...
// GemStone changes END
    ...
    */
// GemStone changes BEGIN
	private int /* boolean */ addToThreadList(Thread me, ContextManager associateCM) {
// GemStone changes END
...
// GemStone changes BEGIN
		// changed to Dequeue from Stack
		final java.util.ArrayDeque<Object> stack;
		int res = IS_ADDED;
 		/* (original code) java.util.Stack stack; */
// GemStone changes END
		if (list instanceof ContextManager) {
			...
			if (threadsCM.activeThread != me) {
				...
				tcl.set(associateCM);
// GemStone changes BEGIN
				return OLD_INACTIVE_CM_OVERWRITTEN;
				/* (original code) return true; */
// GemStone changes END
			}
...
// GemStone changes BEGIN
			// changed to Dequeue from Stack
			stack = new java.util.ArrayDeque<Object>(8);
			/* (original code) stack = new java.util.Stack(); */
// GemStone changes END
			tcl.set(stack);
...
// GemStone changes BEGIN
		  // changed to Dequeue from Stack
		  stack = (java.util.ArrayDeque<Object>)list;
		  if (stack.peekFirst() == associateCM) {
		    res = ALREADY_CURRENT_IS_ADDED;
		  }
			/* (original code) stack = (java.util.Stack) list; */
// GemStone changes END
		}
...
// GemStone changes END
	 ...
	 */
// GemStone changes BEGIN
	// return boolean to indicate whether ContextManager had to be be set
	// (true) or was already set correctly (false)
	public boolean /* void */ setCurrentContextManager(ContextManager cm) {
// GemStone changes END
		...
		if (SanityManager.DEBUG) {
			...
			if (cm.activeThread != null && me != cm.activeThread) {
				SanityManager.THROWASSERT("setCurrentContextManager - mismatch threads - current " + me + " - cm's " + cm.activeThread + " for " + cm +  " with conn " + EmbedConnectionContext.getEmbedConnection(cm));
			}
...
// GemStone changes BEGIN
		final int result;
		if ((result = addToThreadList(me, cm)) < IS_ADDED) {
		  ++cm.activeCount;
		}
// GemStone changes BEGIN
		if (GemFireXDUtils.TraceCM) {
		  SanityManager.DEBUG_PRINT(GfxdConstants
		      .TRACE_CONTEXT_MANAGER, "Set active thread to me "
		      + cm.activeThread + " activeCount=" + cm.activeCount
		      + " result=0x" + Integer.toHexString(result)
		      + " for cm " + ArrayUtils.objectRefString(cm)
		      +  " with conn " + EmbedConnectionContext
		          .getEmbedConnection(cm), new Throwable());
		}
// GemStone changes END
		return (result & ALREADY_CURRENT) != ALREADY_CURRENT;
		/* original code)
...
// GemStone changes BEGIN
		// removing sync since allContexts is already concurrent
		{
		/* (original code)
		synchronized (this) {
		*/
			...
			if (SanityManager.DEBUG) {
				...
				if (SanityManager.TraceMemoryLeak) {
					...
					if (allContexts.size() > 50)
						System.out.println("TraceMemoryLeak:ContextService:allContexts " + allContexts.size());
				}
			}
		}
		...
		*/
			for (Iterator i = allContexts.iterator(); i.hasNext(); ) {
				...
				if (active == null)
					continue;
// GemStone changes BEGIN
				if (active.getThreadGroup() instanceof GemFireThreadGroup
				    && !((GemFireThreadGroup)active.getThreadGroup())
				    .isInterruptible()) {
				  continue;
				}
// GemStone changes END
...
// GemStone changes BEGIN
    ...
    public static void removeContextManager(final ContextManager cm) {
      final ContextService fact = factory;
      if (fact != null) {
    ...
    public final void removeContext(ContextManager cm) {
      final ConcurrentHashSet<ContextManager> contexts = this.allContexts;
      if (contexts != null) {
        contexts.remove(cm);
      }
    }
