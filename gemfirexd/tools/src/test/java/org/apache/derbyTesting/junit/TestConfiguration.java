...
public class TestConfiguration {
    ...
    static {
        ...
        // If gemfirexd.system.home set externally at startup assume
        // running in harness
        if (BaseTestCase.getSystemProperty("gemfirexd.system.home") != null)
            assumeHarness = true;
        ...
        if (!assumeHarness) {
            ...
            (new java.security.PrivilegedAction(){
                public Object run(){
                    BaseTestCase.setSystemProperty("gemfirexd.system.home",
                                                   dsh.getAbsolutePath());
                    return null;
                }
            }
    ...
    public static Test defaultExistingServerDecorator(Test test)
    {
        ...
        {
            return new TestSuite("empty: no network server support in JSR169 (or gemfirexd-client.jar missing).");
        }
    ...
    */
    ...
    {       
        ...
        sqlAuth.setProperty("gemfirexd.sql-authorization", "true");
...
// GemStone changes BEGIN
               url = getNetProtocol(hostName, port);
               /* (original code)
...
// GemStone changes BEGIN
           // do not use a database name with GemFireXD
           /*
...
// GemStone changes BEGIN
    ...
    public static final boolean USE_ODBC_BRIDGE = Boolean
...
// GemStone changes END
    ...
    public void shutdownDatabase()
    {
        try {
            ...
            // GemStone changes BEGIN
            // ignore DB shutdown failure
            //Assert.fail("Database failed to shut down");
        } catch (SQLException e) {
             // see #41296
             BaseJDBCTestCase.assertSQLState("Database shutdown", /*"08006"*/"XJ015" , e);
             // GemStone changes END
        }
    }
   ...
   /**
    ...
    boolean defaultSecurityManagerSetup() {
        // GemStone changes BEGIN
        // turn off SecurityManager, it's a pain and there's no
        // known requirement for it in GemFireXD
        return false;
